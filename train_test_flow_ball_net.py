...
from dataloader.dataloader_bbox_flow_detector_net import read_dataset
...
parser.add_argument('--people_lama_num', action='store_true', help='use people inpaint with LaMa based on number of people')
parser.add_argument('--inpaint_people_num', type=int, default=1, help='number of people threshold for people inpainting')
parser.add_argument('--people_mask_scale', type=float, default=1.0, help='scale for people mask')
parser.add_argument('--inpaint_people_num_sync', action='store_true', help='use people inpaint with LaMa based on number of people sync')
...
parser.add_argument('--add_ball_noise', action='store_true', help='add noise to ball bbox')
parser.add_argument('--ball_noise_sigma', default=5.0, type=float, help='sigma for ball bbox noise')
...
def test_evaluation(train_loader_for_val, test_loader, model, args, save_path):
    ...
    for sample in tqdm(train_loader_for_val, desc="Extracting Train Features for Test"):
        if args.use_flow or args.use_flow_numpy:
            images, activities, _, _, bboxes, _, _ = sample
        else:
            images, activities, _, bboxes, _, _ = sample
        images = images.cuda()
    ...
    for sample in tqdm(test_loader, desc="Extracting Test Features"):
        if args.use_flow or args.use_flow_numpy:
            images, activities, ball_gt, flow, bboxes, _, _ = sample
        else:
            images, activities, ball_gt, bboxes, _, _ = sample
        images = images.cuda()
        input_data = {'images': images, 'bboxes': bboxes}
        ret_dic = model(input_data)
        video_features = ret_dic['video_features']
...
def log_metrics_to_wandb(train_log, test_log=None):
    if train_log is not None:
        wandb.log({
            ...
            "train_loss_net_bbox": train_log['loss_net_bbox'],
            "train_loss_temp_net_bbox": train_log['loss_temp_net_bbox'],
    ...
    if test_log is not None:
        wandb.log({
            ...
            "val_loss_net_bbox": test_log['loss_net_bbox'],
            "val_loss_temp_net_bbox": test_log['loss_temp_net_bbox'],
...
def initialize_wandb(args):
    if args.dataset == 'volleyball':
        if args.pretrained_weights != '':
            project_name = "flow_ball_net_volleyball_finetune"
        else:
            project_name = "flow_ball_net_volleyball"
    elif args.dataset == 'nba':
        if args.pretrained_weights != '':
            project_name = "flow_ball_net_nba_finetune"
        else:
            project_name = "flow_ball_net_nba"
    wandb.init(
...
def train(args, train_loader, model, criterion, optimizer, epoch, lama_model):
    ...
    losses_net_bbox = AverageMeter()
    losses_temp_net_bbox = AverageMeter()
    ...
    for i, sample in enumerate(tqdm(train_loader, desc="Training")):
        ...
        loss_net_bbox = torch.tensor(0.0).cuda()
        loss_net_spatial = torch.tensor(0.0).cuda()
        loss_net_temporal = torch.tensor(0.0).cuda()
        if args.use_flow or args.use_flow_numpy:
            images, _, ball_gt, flow, bboxes, _, net_bboxes = sample
            flow = flow.cuda()
        else:
            images, _, ball_gt, bboxes, _, net_bboxes = sample
        images = images.cuda()
        ball_gt = ball_gt.cuda()
        bboxes = bboxes.cuda()
        net_bboxes = net_bboxes.cuda()
        if args.add_ball_noise:
            ball_gt = add_ball_noise_px_skip_zeros(
                ball_gt,
                sigma_px=args.ball_noise_sigma,
            )
        if args.add_bbox_noise:
            bboxes = add_bbox_center_noise_pixels(
                bboxes,
                sigma_c=args.bbox_noise_sigma,
            )
        ...
        if args.ball_lama or args.people_lama or args.people_lama_num or args.net_lama:
            imgs_flat   = images.view(B * T, C, H, W)   # (B⋅T, C, H, W)
            ...
            if args.net_lama:
                if args.frame_random:
                    do_net = (torch.rand(B * T, device='cuda') < args.inpaint_prob_net)
                elif args.batch_random:
                    batch_mask = [random.random() < args.inpaint_prob_net for _ in range(B)]
                    do_net = torch.zeros(B * T, dtype=torch.bool, device='cuda')
                    for b in range(B):
                        if batch_mask[b]:
                            do_net[b * T : (b + 1) * T] = True
                else:
                    do_net = torch.ones(B * T, dtype=torch.bool, device='cuda')

                if args.dataset == 'volleyball':
                    net_masks = generate_volleyball_net_masks(
                        normalized_batch=imgs_flat,
                        net_bboxes=net_bboxes,
                        scale=args.net_mask_scale,
                        bbox_normalized=True,
                        device='cuda',
                    )  # (B*T, 1, H, W)
                elif args.dataset == 'nba': 
                    net_masks = generate_net_masks(
                        normalized_batch=imgs_flat,
                        net_bboxes=net_bboxes,
                        scale=args.net_mask_scale,
                        bbox_normalized=True,
                        device='cuda',
                        extend_to_top=args.net_extend_to_top,
                        extend_side=args.net_extend_side
                    )  # (B*T, 1, H, W)

                net_masks[~do_net] = 0.0
            else:
                net_masks = torch.zeros(B * T, 1, H, W, device='cuda')

            combined_masks = torch.clamp(ball_masks + people_masks + net_masks, 0.0, 1.0)  # (B⋅T, 1, H, W)
        ...
        if args.net_pred:
            net_bboxes_flat = net_bboxes.view(B*T, 4)
            cx_norm = (net_bboxes_flat[:, 0] + net_bboxes_flat[:, 2]) / 2.0
            cy_norm = (net_bboxes_flat[:, 1] + net_bboxes_flat[:, 3]) / 2.0

            net_bboxes_center_flat = torch.zeros(B*T, 2, device=net_bboxes.device)
            net_bboxes_center_flat[:, 0] = cx_norm
            net_bboxes_center_flat[:, 1] = cy_norm
            net_bboxes_center = net_bboxes_center_flat.view(B, T, 2)   # (B, T, 2)

            valid_net_frame_mask = (net_bboxes_center != 0).any(dim=-1)  # (B, T)
        
        if args.ball_pred:
            valid_frame_mask = (ball_gt != 0).any(dim=-1)      # (B, T)
            valid_frame_mask_flat = valid_frame_mask.view(-1)  # (B*T,)

            if not args.flow_pred:
                if valid_frame_mask_flat.any():
                    if args.net_pred:
                        if not valid_net_frame_mask.view(-1).any():
                            continue
                    ret_dic = model(input_data)
                else:
                    continue
            ...
            valid_net_frame_mask_flat = valid_net_frame_mask.view(-1)
            if not args.flow_pred:
                if valid_net_frame_mask_flat.any():
                    ret_dic = model(input_data)
                else:
                    continue
            else:
        ...
        if args.net_pred:
            net_pred_spatial = ret_dic['pred_net_bbox_spatial']     # (B, T, 2)
            net_pred_temporal = ret_dic['pred_net_bbox_temporal']

            net_pred_spatial_flat = net_pred_spatial.view(-1, net_pred_spatial.size(-1))  # (B*T, 2)
            net_pred_temporal_flat = net_pred_temporal.view(-1, net_pred_temporal.size(-1))

            net_gt_flat = net_bboxes_center.view(-1, net_bboxes_center.size(-1))  # (B*T, 2)

            valid_net_frame_mask_flat = valid_net_frame_mask.view(-1)  # (B*T,)
            if valid_net_frame_mask_flat.any():
                net_pred_spatial_valid = net_pred_spatial_flat[valid_net_frame_mask_flat]
                net_gt_valid          = net_gt_flat[valid_net_frame_mask_flat]

                if args.spatial_net_loss:
                    loss_net_spatial = criterion(net_pred_spatial_valid, net_gt_valid)
                    loss += args.w_net * loss_net_spatial

                if args.temporal_net_loss:
                    net_pred_temporal_valid = net_pred_temporal_flat[valid_net_frame_mask_flat]
                    loss_net_temporal = criterion(net_pred_temporal_valid, net_gt_valid)
                    loss += args.w_net * loss_net_temporal
        ...
        if args.net_pred:
            losses_net_bbox.update(loss_net_spatial.item(), B)
            losses_temp_net_bbox.update(loss_net_temporal.item(), B)
    ...
    train_log = {
        ...
        'loss_net_bbox': losses_net_bbox.avg,
...
def validate(train_loader_for_val, test_loader, model, criterion, epoch, optimizer, scheduler, save_path):
    ...
    for sample in tqdm(train_loader_for_val, desc="Extracting Train Features"):
        if args.use_flow or args.use_flow_numpy:
            images, activities, _, _, bboxes, _, net_bboxes = sample
        else:
            images, activities, _, bboxes, _, net_bboxes = sample
    ...
    for sample in tqdm(test_loader, desc="Extracting Test Features"):
        ...
        loss_net_spatial = torch.tensor(0.0, device=device)
        loss_net_temporal = torch.tensor(0.0, device=device)

        if args.use_flow or args.use_flow_numpy:
            images, activities, ball_gt, flow, bboxes, _, net_bboxes = sample
            flow = flow.to(device)
        else:
            images, activities, ball_gt, bboxes, _, net_bboxes = sample

        images = images.to(device)
        ball_gt = ball_gt.to(device)
        bboxes = bboxes.to(device)
        net_bboxes = net_bboxes.to(device)
        ...
        if args.ball_pred:
            ...
            valid_frame_mask = (ball_gt != 0).any(dim=-1)  # (B, T)
            bbox_pred_flat = bbox_pred.view(-1, bbox_pred.size(-1))               # (B*T, 4)
            temporal_bbox_pred_flat = temp_bbox_pred.view(-1, temp_bbox_pred.size(-1))
            ball_gt_flat = ball_gt.view(-1, ball_gt.size(-1))                     # (B*T, 4)
            valid_frame_mask_flat = valid_frame_mask.view(-1)                     # (B*T,)
        ...
        # ===== Net loss =====
        if args.net_pred:
            net_bboxes_flat = net_bboxes.view(B * T, 4)  # (B*T, 4)
            cx_norm = (net_bboxes_flat[:, 0] + net_bboxes_flat[:, 2]) / 2.0
            cy_norm = (net_bboxes_flat[:, 1] + net_bboxes_flat[:, 3]) / 2.0
            ...
            net_pred_spatial = ret_dic['pred_net_bbox_spatial']       # (B, T, 2) 
            net_pred_temporal = ret_dic['pred_net_bbox_temporal']     # (B, T, 2) 
...
def main():
    ...
    if args.pretrained_weights != '':
        exp_name = '[%s]_dino_flow_ball_net_finetune<%s>' % (args.dataset, time_str)
        save_path = './finetune_result/%s' % exp_name
    else:
        exp_name = '[%s]_dino_flow_ball_net<%s>' % (args.dataset, time_str)
    ...
    train_set, train_set_for_val, test_set, _, _, _, train_frames, test_frames = read_dataset(args)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # train_log = train(args, train_loader, model, criterion, optimizer, epoch, lama_model, train_frames, test_frames)
