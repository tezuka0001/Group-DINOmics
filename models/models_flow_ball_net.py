...
class Ball_detect_model(nn.Module):
    def __init__(self, args):
        ...
        if self.net_pred:
            self.net_bbox_head = nn.Sequential(
                nn.Linear(self.input_dim, self.input_dim),
                nn.ReLU(),
                nn.Linear(self.input_dim, 2),
            )
            self.cond_net_bbox_head = nn.Sequential(
                nn.Linear(self.input_dim, self.input_dim),
                nn.ReLU(),
                nn.Linear(self.input_dim, 2),
            )
    ...
    def forward(self, input_data):
        ...
        if self.net_pred:
            pred_net_bbox_spatial = self.net_bbox_head(cls_token)
            ret_dic['pred_net_bbox_spatial'] = pred_net_bbox_spatial
        ...
        if self.net_pred:
            ...
            if self.loc_guide == 'spatial_temporal_loc' or self.loc_guide == 'temporal_loc':
                ...
                cond_video_feature = video_feature_expand
            cond_video_feature = cond_video_feature.contiguous().view(B*T, self.input_dim)
            pred_net_bbox_temporal = self.cond_net_bbox_head(cond_video_feature)
            ret_dic['pred_net_bbox_temporal'] = pred_net_bbox_temporal
