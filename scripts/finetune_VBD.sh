...
echo "degug1"
CUDA_VISIBLE_DEVICES=$gpu_id python train_test_flow_ball_finetune.py "${common_params[@]}" "${other_params[@]}" "${epochs[@]}" "${mask_params[@]}" "${finetune_params[@]}" "${pattern1[@]}" "${detector_params[@]}"
echo "degug2"
# CUDA_VISIBLE_DEVICES=$gpu_id python train_test_flow_ball_finetune.py "${common_params[@]}" "${other_params[@]}" "${epochs[@]}" "${mask_params[@]}" "${finetune_params[@]}" "${pattern2[@]}" "${detector_params[@]}"
# echo "degug3"
# CUDA_VISIBLE_DEVICES=$gpu_id python train_test_flow_ball_finetune.py "${common_params[@]}" "${other_params[@]}" "${epochs[@]}" "${mask_params[@]}" "${loss_weight[@]}" "${finetune_params[@]}" "${pattern1[@]}" "${pattern2[@]}" "${detector_params[@]}"
# echo "degug4"